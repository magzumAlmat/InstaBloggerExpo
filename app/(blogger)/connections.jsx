      ...
      <BlurView tint="dark" intensity={40} style={{ borderRadius: 22, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', padding: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
          {brand.avatar_url && brand.avatar_url !== '/uploads/avatars/default.jpg' ? (
            <Image source={{ uri: `http://172.20.10.7:3000${brand.avatar_url}` }} style={{ width: 56, height: 56, borderRadius: 28 }} />
          ) : (
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '800' }}>@{brand.ig_username}</Text>
            <Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 2 }}>{brand.category}</Text>
          </View>
    ...
    <View style={{ flex: 1, backgroundColor: '#0A061E', paddingTop: 100 }}>
      {connections.length === 0 ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32 }}>
          ...
          <Text style={{ color: '#fff', fontSize: 20, fontWeight: '800', textAlign: 'center', marginTop: 20, lineHeight: 28 }}>Нет активных чатов</Text>
          <Text style={{ color: 'rgba(255,255,255,0.4)', fontSize: 15, textAlign: 'center', marginTop: 8, lineHeight: 22 }}>Взаимные свайпы появятся здесь, и вы сможете начать общение.</Text>
        </View>
      ) : (
        <FlatList
          data={connections}
